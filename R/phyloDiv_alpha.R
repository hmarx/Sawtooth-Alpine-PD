...
saw.sesmpd.phylonull <- ses.mpd(sawMiseq$comm, cophenetic.phylo(sawMiseq$phy), null.model = "phylogeny.pool", abundance.weighted = FALSE, runs = 10000)
write.csv(saw.sesmpd.phylonull, file="output/08_PhyloDiversity/MiSeq/alpha/static/saw.sesmpd.phylonull.csv")

saw.sesmntd.phylonull <- ses.mntd(sawMiseq$comm, cophenetic.phylo(sawMiseq$phy), null.model = "phylogeny.pool", abundance.weighted = FALSE, runs = 10000)
write.csv(saw.sesmntd.phylonull, file="output/08_PhyloDiversity/MiSeq/alpha/static/saw.sesmntd.phylonull.csv")


# Pruned phylogeny and community matrix for each clade 
sawtooth.asterales <- pruneCladeTaxonomyLookup(tip.labels = sawMiseq$phy$tip.label, tax, level = "order", taxonomy = "Asterales")
pruned.tree.saw.aster <-drop.tip(sawMiseq$phy, sawMiseq$phy$tip.label[!sawMiseq$phy$tip.label %in% sawtooth.asterales])
...
sawtooth.caryophyllales <- pruneCladeTaxonomyLookup(tip.labels = sawMiseq$phy$tip.label, tax, level = "order", taxonomy = "Caryophyllales")
pruned.tree.saw.caryo <-drop.tip(sawMiseq$phy, sawMiseq$phy$tip.label[!sawMiseq$phy$tip.label %in% sawtooth.caryophyllales])
...
sawtooth.poales <- pruneCladeTaxonomyLookup(tip.labels = sawMiseq$phy$tip.label, tax, level = "order", taxonomy = "Poales")
pruned.tree.saw.poales <-drop.tip(sawMiseq$phy, sawMiseq$phy$tip.label[!sawMiseq$phy$tip.label %in% sawtooth.poales])
...
sawtooth.brassicales <- pruneCladeTaxonomyLookup(tip.labels = sawMiseq$phy$tip.label, tax, level = "order", taxonomy = "Brassicales")
pruned.tree.saw.brassicales <-drop.tip(sawMiseq$phy, sawMiseq$phy$tip.label[!sawMiseq$phy$tip.label %in% sawtooth.brassicales])
...
sawtooth.ericales <- pruneCladeTaxonomyLookup(tip.labels = sawMiseq$phy$tip.label, tax, level = "order", taxonomy = "Ericales")
pruned.tree.saw.ericales <-drop.tip(sawMiseq$phy, sawMiseq$phy$tip.label[!sawMiseq$phy$tip.label %in% sawtooth.ericales])
...
sawtooth.saxifragales <- pruneCladeTaxonomyLookup(tip.labels = sawMiseq$phy$tip.label, tax, level = "order", taxonomy = "Saxifragales")
pruned.tree.saw.saxifragales <-drop.tip(sawMiseq$phy, sawMiseq$phy$tip.label[!sawMiseq$phy$tip.label %in% sawtooth.saxifragales])
...
sawtooth.lamiales <- pruneCladeTaxonomyLookup(tip.labels = sawMiseq$phy$tip.label, tax, level = "order", taxonomy = "Lamiales")
pruned.tree.saw.lamiales <-drop.tip(sawMiseq$phy, sawMiseq$phy$tip.label[!sawMiseq$phy$tip.label %in% sawtooth.lamiales])
...
phylogeny.poolSESmntd <- rbind((saw.sesmntd.phylonull %>%  mutate(summits = rownames(saw.sesmntd.phylonull), metric = "mntd", clade = "Spermatophyta", sig=ifelse(saw.sesmntd.phylonull[,"mntd.obs.p"] <= 0.05 | saw.sesmntd.phylonull[,"mntd.obs.p"] > 0.95, 1,0))),
                               ...
                               (ericales.sesmntd.phylonull %>%  mutate(summits = rownames(ericales.sesmntd.phylonull), metric = "mntd", clade = "Ericales", sig=ifelse(ericales.sesmntd.phylonull[,"mntd.obs.p"] <= 0.05 | ericales.sesmntd.phylonull[,"mntd.obs.p"] > 0.95, 1,0))))
names(phylogeny.poolSESmntd)[2:7] <- c("obs", "rand.mean", "rand.sd", "obs.rank", "obs.z", "obs.p")

phylogeny.poolSESmpd <- rbind((saw.sesmpd.phylonull %>%  mutate(summits = rownames(saw.sesmpd.phylonull), metric = "mpd", clade = "Spermatophyta", sig=ifelse(saw.sesmpd.phylonull[,"mpd.obs.p"] <= 0.05 | saw.sesmpd.phylonull[,"mpd.obs.p"] > 0.95, 1,0))),
                              ...
                              (brassicales.sesmpd.phylonull %>%  mutate(summits = rownames(brassicales.sesmpd.phylonull), metric = "mpd", clade = "Brassicales", sig=ifelse(brassicales.sesmpd.phylonull[,"mpd.obs.p"] <= 0.05 | brassicales.sesmpd.phylonull[,"mpd.obs.p"] > 0.95, 1,0))),
                               (ericales.sesmpd.phylonull %>%  mutate(summits = rownames(ericales.sesmpd.phylonull), metric = "mpd", clade = "Ericales", sig=ifelse(ericales.sesmpd.phylonull[,"mpd.obs.p"] <= 0.05 | ericales.sesmpd.phylonull[,"mpd.obs.p"] > 0.95, 1,0))))
...
phylogeny.poolSES$clade <- factor(phylogeny.poolSES$clade, levels = c("Ericales", "Brassicales", "Poales", "Caryophyllales", "Asterales", "Spermatophyta"))
phylogeny.poolSES <- cbind(phylogeny.poolSES, pool = rep(x = "Sawtooth Alpine", times = nrow(phylogeny.poolSES)))
#write.csv(phylogeny.poolSES, file="output/08_PhyloDiversity/MiSeq/alpha/static/alpine.phylogeny.pool.SES.csv")
